...
// "Plan today with Claude" — the same PlanCard used on the Roadmaps and Projects
// tabs, framed for the day. Auto-expands on the first visit of each day (tracked
// in localStorage), then sits collapsed so it's one tap away without nagging.
...
function PlanTodayCard({ ctx, day }) {
  ...
  return (
    <PlanWithClaudeCard
      ctx={ctx}
      title="Plan today with Claude"
      blurb="Copy the prompt, talk through your day with Claude — your roadmaps and tasks, plus anything else you want to get done today — then paste the reply back to save it."
      defaultOpen={notSeenToday}
      onOpen={markSeen}
    />
  );
}
